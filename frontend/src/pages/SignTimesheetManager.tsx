...
const SignTimesheetManager = () => {
    ...
    apiFetch<SignInfo>(`/api/timesheets/sign-request/${token}/info`)
      .then((data) => {
        if (data.status !== "employee_signed") {
          setError("Este link não aguarda assinatura do gestor.");
        } else {
          setInfo(data);
        }
      })
  ...
  const handleSubmit = async () => {
    ...
    setSubmitting(true);
    try {
      await apiFetch(`/api/timesheets/sign-request/${token}/manager-sign`, {
        method: "POST",
        body: { token, manager_signature: managerSignature },
      });
      setDone(true);
    } catch (e: unknown) {
  ...
  if (done) return (
    <div className="min-h-screen flex flex-col items-center justify-center gap-4 px-4 text-center">
      ...
      <p className="text-muted-foreground">O funcionário foi notificado e o PDF está disponível para download.</p>
    </div>
  );
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col items-center py-10 px-4">
      <div className="w-full max-w-lg space-y-6">
        ...
        {info?.employeeSignature && (
          <div className="border rounded-lg p-4 bg-card">
            <p className="text-sm font-medium mb-2">Assinatura de {info.employeeName}:</p>
            <img
              src={info.employeeSignature}
        ...
        <div className="border rounded-lg p-4 bg-card space-y-3">
          <p className="text-sm font-medium flex items-center gap-2">
            <Pen className="h-4 w-4" /> Sua assinatura (gestor)
          </p>
          <canvas
            ref={canvasRef}
            width={600}
...
export default SignTimesheetManager;
